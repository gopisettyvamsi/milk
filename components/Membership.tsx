...
const MembershipCard = ({ plan, isPopular = false, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 border border-white/20 overflow-hidden flex flex-col group ${
        isPopular ? "ring-4 ring-purple-400/50 scale-105 transform" : ""
      }`}
    >
      {/* Popular Tag */}
      {isPopular && (
      ...
      <div className="relative p-8 text-center border-b border-white/20">
        ...
        <h3 className={`text-3xl font-bold mb-3 group-hover:text-transparent group-hover:bg-gradient-to-r transition-all duration-300 ${
          isPopular 
      ...
      <div className="px-8 py-6 flex-1 bg-gradient-to-br from-gray-50/30 to-white/50">
        <ul className="space-y-4">
          {plan.features.map((feature, featureIndex) => (
            ...
            >
              <div className={`w-6 h-6 rounded-full flex items-center justify-center mr-4 mt-0.5 group-hover/feature:scale-110 transition-transform duration-200 ${
                isPopular 
      ...
      <div className="p-8 bg-gradient-to-br from-white/50 to-gray-50/50 border-t border-white/20">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-300 hover:shadow-lg group/button flex items-center justify-center ${
            isPopular
