...
import { DownloadIcon, Eye, Loader, Loader2 } from 'lucide-react';
...
const PaymentsHistory: React.FC = () => {
  ...
  const handlePageChange = (page: number): void => {
    if (page < 1 || page > totalPages) return;  // Prevent navigating to invalid pages
    setCurrentPage(page);
  };
  ...
  // ✅ Trigger download from API route
  const handleDownload = async (transactionId: string) => {
    try {
      setDownloadingId(transactionId); // start loader

      const res = await fetch(`/api/payments/receipt/${transactionId}/pdf`);
  ...
  return (
    <UserLayout>
      <div className="container mx-auto px-2 sm:px-4 p-6 bg-white">
        <div className="mb-8">
              ...
              <tbody className="divide-y divide-gray-200">
                {currentRecords.map((payment) => (
                  <tr key={payment.payment_id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-30 py-4 whitespace-nowrap">
                      <button
                        onClick={() => {
                          if (
                            ...
                            window.location.href = `/payments/receipt/${payment.transaction_id}`;
                          }
                        }}
                        disabled={
                          payment.payment_status === "FAILED" ||
                        ...
                        className={`flex items-center justify-center px-3 py-1.5 rounded-md text-sm font-medium transition ${payment.payment_status === "FAILED" || payment.payment_status === "PENDING"
                        ...
                        title={
                          payment.payment_status === "FAILED"
                            ? "View disabled — payment failed"
                            : payment.payment_status === "PENDING"
                              ? "View disabled — payment pending"
                              : "View receipt"
                        }
                      >
                        View Receipt
